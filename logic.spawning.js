roleSpawning = {
    ...
    run: function(spawn) {
        ...
        var ammountOfHarvesters = 3;
        var ammountOfUpgraders = 4;
        var ammountOfBuilders = 1;
        var ammountOfRepairmen = 0;

    	// Spawn harvester if there are less than 2 harvesters.
    	if(totalHarvesters.length < ammountOfHarvesters && totalEnergyInRoom > 500) {
    	...
    	if(totalUpgraders.length < ammountOfUpgraders && totalEnergyInRoom > 500) {
        ...
        // Spawn Upgraders if there are less than 2 upgraders.
        if(totalBuilders.length < ammountOfBuilders && totalEnergyInRoom > 500) {
        ...
        if (totalRepairmen.length < ammountOfRepairmen && totalEnergyInRoom > 500)
        {
